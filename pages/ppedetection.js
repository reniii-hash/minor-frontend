...
const PPEDetection = () => {
  ...
  const checkComplianceAndAlert = (detections) => {
    ...
    const criticalPPE = ["helmet"] // Helmet is critical
  ...
  const detectInImage = async (imageSrc) => {
    ...
    try {
      ...
      img.onload = async () => {
        ...
        // Check compliance for uploaded image
        const hasHelmet = filteredDetections.some((d) => d.class === "helmet")
        const hasVest = filteredDetections.some((d) => d.class === "safety_vest")

        if (!hasHelmet) {
          setSystemStatus("⚠️ CRITICAL: No Helmet Detected!")
          logViolation("No Helmet Detected")
        } else if (!hasVest) {
          setSystemStatus("⚠️ WARNING: No Safety Vest Detected")
          logViolation("No Safety Vest Detected")
        } else {
